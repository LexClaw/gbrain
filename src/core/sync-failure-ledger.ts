...
export interface SyncFailure {
  ...
  attempts: number;
  /** Legacy mirror of attempts for pre-#1939 readers. */
  attempt_count?: number;
  /** Timestamp of the most recent duplicate/retry failure. */
...
export function classifyErrorCode(errorMsg: string): string {
  ...
  // write or post-write embedding path failed and must be retried/alarmed rather
  // than silently absorbed by sync-failure deduplication.
...
function _normalizeRow(raw: Record<string, unknown>): SyncFailure {
  ...
  const attempts =
    typeof raw.attempts === 'number' && Number.isFinite(raw.attempts) && raw.attempts > 0
      ...
      : typeof raw.attempt_count === 'number' && Number.isFinite(raw.attempt_count) && raw.attempt_count > 0
        ? Math.floor(raw.attempt_count)
        : 1;
  const row: SyncFailure = {
    ...
    alarmed: raw.alarmed === true || attempts >= 2,
    alarmed_at: typeof raw.alarmed_at === 'string' ? raw.alarmed_at : (attempts >= 2 ? ts : undefined),
    state,
    resolved_at:
  ...
  return withLedgerLock(() => {
    ...
    for (const f of failures) {
      ...
      if (ex && ex.state === 'open') {
        ex.attempts += 1;
        ex.ts = now;
        ex.last_attempt_at = now;
        if (ex.attempts >= 2 && !ex.alarmed) {
          ex.alarmed = true;
          ex.alarmed_at = now;
        }
        ex.commit = commit;
...
export function decideSyncFailureSeverity(args: {
  ...
  const alarmed = unresolved.some(e => e.alarmed === true);
  ...
  const status: 'warn' | 'fail' = alarmed || open >= 10 || blockedTooLong ? 'fail' : 'warn';
