...
export interface DeleteFactsForPageOpts {
   ...
   * `fence-origin` deletes only NULL/empty/`fence%` source rows, protecting
   * every non-fence writer at that page coordinate. Omitted preserves the
   * legacy unscoped delete behavior, unless excludeSourcePrefixes is set.
   */
  sourceScope?: 'fence-origin';
  /**
   * Upstream #1928 compatibility: protect rows whose `source` starts with any
   * prefix (for example `cli:` conversation facts) from an otherwise unscoped
   * wipe. This can also be combined with sourceScope for belt-and-suspenders.
   */
