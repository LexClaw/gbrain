...
export async function doctorReportRemote(engine: BrainEngine): Promise<DoctorReport> {
  ...
  try {
    ...
    const alarmedFailures = entries.filter(f => f.alarmed);
    ...
    const msg =
      sev.unresolved === 0
        ...
        : `${alarmedFailures.length > 0 ? `ALARM: ${alarmedFailures.length} unrecovered sync failure(s) survived retry. ` : ''}${sev.unresolved} unresolved sync failure(s)` +
...
export async function buildChecks(
  ...
  try {
    ...
    const causeStr = `runtime=${summary.by_cause.runtime_error} oom=${summary.by_cause.oom_or_external_kill} sigpipe=${summary.by_cause.sigpipe} rss=${summary.by_cause.rss_watchdog} unknown=${summary.by_cause.unknown} legacy=${summary.by_cause.legacy}${summary.by_cause.rss_watchdog > 0 ? ' (see worker_oom_loop)' : ''}`;
  ...
  try {
    ...
    if (unresolved.length > 0) {
      ...
      const alarmedFailures = unresolved.filter(f => f.alarmed);
      ...
      checks.push({
        ...
        status: sev.status,
        message:
          `${alarmPrefix}${unresolved.length} unresolved sync failure(s) [${codeBreakdown}]` +
          (sev.auto_skipped > 0 ? ` — ${sev.auto_skipped} auto-skipped (pages NOT indexed)` : '') +
          `. ${preview}` +
