...
const CLI_ONLY = new Set(['init', 'reinit-pglite', 'upgrade', 'post-upgrade', 'check-update', 'integrations', 'publish', 'check-backlinks', 'lint', 'report', 'import', 'export', 'files', 'embed', 'serve', 'call', 'config', 'doctor', 'migrate', 'eval', 'sync', 'extract', 'extract-conversation-facts', 'enrich', 'features', 'capabilities', 'autopilot', 'graph-query', 'jobs', 'agent', 'apply-migrations', 'skillpack-check', 'skillpack', 'resolvers', 'integrity', 'repair-jsonb', 'orphans', 'sources', 'mounts', 'dream', 'check-resolvable', 'routing-eval', 'skillify', 'smoke-test', 'providers', 'storage', 'repos', 'code-def', 'code-refs', 'reindex', 'reindex-code', 'reindex-frontmatter', 'code-callers', 'code-callees', 'frontmatter', 'auth', 'ingest', 'friction', 'claw-test', 'book-mirror', 'takes', 'think', 'salience', 'anomalies', 'transcripts', 'models', 'remote', 'recall', 'forget', 'edges-backfill', 'cache', 'ze-switch', 'founder', 'brainstorm', 'lsd', 'schema', 'capture', 'onboard', 'conversation-parser', 'status', 'connect', 'skillopt', 'quarantine', 'self-upgrade', 'recovery']);
...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'recovery' && (args.includes('--help') || args.includes('-h') || ['manifest', 'approval-verify'].includes(args[0] ?? ''))) {
    const { runRecovery } = await import('./commands/recovery.ts');
    await runRecovery(null as any, args);
    return;
  }
  ...
  try {
    switch (command) {
      ...
      case 'capabilities': {
        const { runCapabilities } = await import('./commands/capabilities.ts');
        await runCapabilities(engine, args);
        break;
      }
      case 'autopilot': {
